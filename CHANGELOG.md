...
Aligns the SDK with ERC-8262 main (`a338616`) plus its regenerated `0x07` / `0x09` verifiers ([xochi-fi/ERC-8262#19](https://github.com/xochi-fi/ERC-8262/pull/19)), replaces the retired fee schedule, and fixes the findings of the 0.3.0 adversarial review. Every entry under Breaking says how to migrate; a consolidated snippet follows at the end.
...
- **`COMPLIANCE_MULTI_SIGNED` (proof type `0x09`)** -- M-of-N multi-provider signed compliance. Bundles up to `MAX_PROVIDERS_MULTI = 5` parallel signer slots; M of them must each produce a valid secp256k1 signature over a slot-specific Pedersen digest AND each must individually attest the subject is below the jurisdiction's high-risk floor. Trust upgrade over `0x07` (one signer "compliant" vs. M independent signers "compliant", AND-aggregated). Mirrors the on-chain validator and verifier shipped in `erc-xochi-zkp` 2026-05-14.
  ...
  - `buildComplianceMultiSignedInputs` -- input builder. Takes `slots: (MultiSignedSlot | null)[]` with length exactly 5; `null` slots get the inactive-slot witness padding (`weight_sum = 1`, `weights = [1, 0..0]`, `signals = [0; 8]`, pubkey `2G` with signature `r = s = 1`) automatically.
...
- `proveComplianceMultiSigned` failed witness generation whenever any slot was `null` (`Failed to solve blackbox function: ecdsa_secp256k1, reason: Signature provided for ECDSA verification is zero`), so only all-5-active bundles could prove. The circuit gates the inactive slot's verify result but still runs the secp256k1 blackbox, which rejects a zero signature or off-curve key. `null` slots are now padded with pubkey `2G` and `r = s = 1`, which acvm_js and bb both evaluate. A slot is still active only when its public `signer_pubkey_hash` is non-zero, so padding never counts toward M. No circuit or verifier change.
